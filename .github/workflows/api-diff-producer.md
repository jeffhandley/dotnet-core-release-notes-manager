      ...
      - name: Publish API diff PR from agent output
        ...
        run: |
          ...
          push_branch() {
            ...
            changed=$(git diff --cached --name-only -- '*.md' 2>/dev/null || true)
            if [ -n "$changed" ]; then
            ...
            git diff --cached --quiet && { echo "::notice::$branch delta is empty after normalization; nothing to push"; return 0; }
...
`[System.ObsoleteAttribute]`); and (2) record it in your update comment and the PR body's
**Feedback applied** section. You do **not** need to react to or otherwise mark the comments: the
PR body's `generated_at` is the processed-through marker, and it advances every run, so feedback
posted before this run is never re-processed.
