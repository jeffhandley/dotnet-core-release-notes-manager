    ...
    steps:
      ...
      - name: Publish API diff PR from agent output
        ...
        run: |
          ...
          push_branch() {
            local branch="$1" bundle="$2" changed md disallowed allow
            ...
            # expected folders plus the two global ApiDiff*ToExclude.txt files. Reject
            # anything else and refuse to publish.
            allow='^release-notes/[0-9]+\.[0-9]+/(preview/[^/]+|[0-9]+\.[0-9]+\.0)/api-diff/.+$|^release-notes/ApiDiff(Attributes|Assemblies)ToExclude\.txt$'
            disallowed=$(git diff --name-only "origin/main...$branch" | grep -vE "$allow" || true)
...
- `existing_pr_number` — the open marker PR to update (empty = open a new one)
- `produce` — **if `false`, STOP: write no manifest and open no PR**
- `noop` — **if `true`, STOP: nothing changed (build unchanged, no feedback); write no manifest and post no comment**
- `report_count`, `temp_excluded_attributes`, `generated_at`
...
`COLLABORATOR`** (`authorAssociation`); skip bots. Route each exclusion request by kind, using
the apidiff file format — **attributes as `T:<FullyQualifiedTypeName>`** (one per line),
**assemblies as the bare assembly name** (no extension):

- **Permanently** exclude an attribute (drop it from every diff) → append `T:<FullName>` to the
  global `permanent_attributes_file`.
- **Temporarily** exclude an attribute (just this diff) → append `T:<FullName>` to the per-report
  `temporary_attributes_file`. (The preload merges permanent + temporary for generation.)
...
For every exclusion you apply, also: (1) remove the now-excluded entries from the current `.md`
reports under `content_dir` so the PR reflects it immediately (reports show the display name, e.g.
`[System.ObsoleteAttribute]`); (2) leave an `:eyes:` reaction on the comment you processed; and
(3) record it in your update comment and the PR body's **Feedback applied** section.

Do not act on feedback that asks for prose, changes the diff semantics, or excludes real API
changes. Leave ambiguous/out-of-scope feedback for a human and say so.

## 3. Stage the branch — allowed files only

You may add/modify **only** these paths (the publish job rejects anything else):

- the api-diff reports under `content_dir` (includes the per-report `ApiDiffAttributesToExclude.txt`)
...
# No-op guard: if nothing actually changed vs the branch, STOP — write no manifest, post no comment.
git diff --cached --quiet "origin/<target_branch>" 2>/dev/null && { echo "no changes; no-op"; exit 0; }
git commit -m "API diff <prev_vm> -> <cur_vm>"
```

Commit only the allowed files. If `git diff --cached --quiet` shows no change versus the existing
branch, this is a no-op: **write no manifest and post no comment.**
...
5. A fenced ```yaml``` block:
   ...
   temporary_attributes_excluded: [<the per-report temporary attribute exclusions, T: form>]
   ```

6. **End the body** with the hidden identity marker on its own last line, verbatim from
   `target.json`: `<!-- api-diff:<prev_vm>_to_<cur_vm> -->`
...
- `produce == false` **or** `noop == true` -> no manifest, no PR, no comment.
- Add/modify **only** the allowed files (reports under `content_dir`, the per-report
  `ApiDiffAttributesToExclude.txt`, and the two global `release-notes/ApiDiff*ToExclude.txt`
  files). One PR per diff, identified by the marker.
